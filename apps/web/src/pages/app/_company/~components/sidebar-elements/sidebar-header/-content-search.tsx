...
import {
	...
	type NavigationItem,
} from "@/constants/_app/company/dashboard-navigation";
...
export function ContentSearch() {
	...
	const handleSelect = (item: NavigationItem) => {
		...
		if (companySlug) {
			finalUrl = finalUrl.replace(":companySlug", companySlug);
		}
	...
	return (
		<>
			<Button
				className="mr-4 w-full cursor-pointer border border-border bg-surface-paper-soft px-3.5 py-2.5 text-foreground-tertiary transition-colors hover:bg-surface-paper-soft/80 hover:text-foreground-tertiary group-data-[collapsible=icon]:w-8 group-data-[collapsible=icon]:justify-center group-data-[collapsible=icon]:px-0"
			...
			>
				...
				<kbd className="pointer-events-none ml-auto flex h-5 select-none items-center gap-1 rounded border-border bg-[#f5efe3] px-1.5 font-medium font-mono text-[10px] text-foreground-tertiary opacity-100 group-data-[collapsible=icon]:hidden">
