...
const companyAddressStepSchema = z.object({
	zipcode: z
		.string()
...
export type CompanyAddressStepValues = z.infer<typeof companyAddressStepSchema>;

interface CompanyAddressStepProps {
	initialData?: CompanyAddressStepValues;
	onBack: (partalData: CompanyAddressStepValues) => void;
	onComplete: (data: CompanyAddressStepValues) => void;
}
...
export function CompanyAddressStep({
	...
	const form = useForm({
		...
		validators: {
			onSubmit: companyAddressStepSchema,
		},
		onSubmit: ({ value }) => {
			onComplete(value);
		},
		...
		<Card className="p-8">
			<div className="flex flex-col gap-6">
				<div className="flex flex-col gap-2">
					<span className={LABEL_CLASS}>Passo 3 de 5 · ~2 minutos</span>
					...
					<p className="font-inter text-[15px] text-foreground-secondary">
						Nos informe seu endereço para que possamos ofertar os melhores
						serviços aos nossos usuários.
					</p>
				</div>

				<form
					id="sign-up-company-address-form"
					onSubmit={(e) => {
						e.preventDefault();
						form.handleSubmit();
					}}
				...
				<div className="flex items-center justify-between">
					...
					<Button
						className="gap-2"
						form="sign-up-company-address-form"
						type="submit"
					>
